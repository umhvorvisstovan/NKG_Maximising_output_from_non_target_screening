...
Figure \@ref(fig:samplingmap) depicts the Nordic sampling stations, further information on each sample can be found in Table \@ref(tab:samplingtable) in the Appendix.
...
The compounds detected by GCxGC-MS (in biota) will be quantified to the extent possible (without internal standards per se, ie. crude estimate from peak area).
...
gcgc_wide <- gcgc_table %>%  
  ...
  rename(SusDat = Norman_SusDat_ID,
...
gcgc_wide %>%
  ...
  kbl(booktabs = T, linesep = "", escape = F, valign = "c",
      caption = "Results of the improved methodology in semi-quantification of the GC-MS detected compounds in the biota samples",
...
```{r stellan, fig.cap="The figure describes the rationale for using a curated chemicals list for identifying a substance. (Stellan Fisher, presented at CityLeaks seminar, Malmø 2018)"}

knitr::include_graphics("figures/Stellan.png")
```
...
The reanalysis resulted in just over 15000 unique feature identifications and of those 127 features were identified to ID Level 2 (Probable Structure, by library/diagnostic evidence) [@Schymanski2015], see Table \@ref(tab:tab1).
...
tabeffluent %>%
  arrange(Name) %>%
  kable(booktabs = T, linesep = "", escape = F, valign = "c",
        caption = "Features identified at ID Level 2 in effluent samples using deconvoluting and library match algorithm",
...
sample_info %>% 
  ...
  rename(Matrix = matrix,
         Site = site,
         Location = location) %>% 
  kable(digits = 3, booktabs = T, linesep = "", caption = "Sampling stations") %>% 
...
# Results from Nordic Suspect Screening, further curated
...
initialwide %>%
  ...
  arrange(Name) %>%
  kable(booktabs = T, linesep = "", escape = F, valign = "c", 
        caption = paste0("Detected compounds in the effluent samples"),
...
initialwide %>%
  ...
  arrange(Name) %>%
  kable(booktabs = T, linesep = "", escape = F, valign = "c", 
        caption = paste0("Detected compounds in the biota samples"),
        col.names = linebreak(c("Molecular\nFormula", "Name", "CAS", "Norman\nSusDat ID",
                                "ID\nLevel", country_labels, 
                                "Comp. \nInfo", "Potential \nSource"),
                              align = "l"),
...
initialwide %>%
  ...
  arrange(Name) %>%
  kable(booktabs = T, linesep = "", escape = F, valign = "c", 
        caption = paste0("Detected compounds in the sediment samples"),
        col.names = linebreak(c("Molecular\nFormula", "Name", "CAS", "Norman\nSusDat ID",
                                "ID\nLevel", country_labels, 
                                "Comp. \nInfo", "Potential \nSource"),
                              align = "l"),
